...
import { Search, Calendar as CalendarIcon, Clock, User, CheckCircle2, XCircle, CalendarClock, Activity, Loader2 } from "lucide-react"
...
/**
 * Try to get a valid Date from the appointment.
 * Priority: timeSlot (always ISO) → time (may be ISO or "HH:MM - HH:MM")
 */
function resolveDate(apt: any): Date | null {
  // 1. timeSlot is always a proper ISO string when present
  if (apt.timeSlot) {
    const d = new Date(apt.timeSlot);
    if (!isNaN(d.getTime())) return d;
  }
  // 2. time field — only valid if it looks like an ISO string
  if (apt.time && apt.time.includes("T")) {
...
function formatDisplayTime(apt: any, date: Date | null): string {
  if (date) {
    return date.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
  }
  // Plain string like "12:00 - 13:00" — use as-is
  return apt.time ?? "—";
}
...
function getDateGroup(date: Date | null): string {
  ...
  const aptStr = date.toISOString().split("T")[0];
  if (aptStr === today) return "Today";
  if (aptStr === tomorrow) return "Tomorrow";
  return "Upcoming";
}
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      const processed = data.map((apt: any) => {
        const date = resolveDate(apt);
        const dateGroup = getDateGroup(date);
        const isLive = dateGroup === "Today" && date !== null && Math.abs(date.getTime() - now.getTime()) < 3600000;

        return {
          ...apt,
          _resolvedDate: date,        // raw Date for sorting/logic
          dateGroup,
          displayTime: formatDisplayTime(apt, date),
      ...
      console.error("Failed to fetch appointments:", error?.response?.status, error?.response?.data, error?.message)
                ...
                <div className="grid gap-4">
                  {groupAppointments.map((apt: any) => {
                    ...
                    return (
                      <div
                        key={apt.id}
                        className={`group relative flex flex-col lg:flex-row lg:items-center gap-5 p-4 lg:p-6 rounded-2xl border bg-white dark:bg-slate-800/90 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${isCompleted || isCancelled
                            ? "opacity-50 grayscale-[0.2] hover:grayscale-0 hover:opacity-100 bg-slate-50/50 dark:bg-slate-900/50"
                            : "border-slate-100 dark:border-slate-700/50"
                          } ${isLive ? "border-emerald-300 dark:border-emerald-600/50 shadow-emerald-500/10" : ""}`}
                      >
                        {isLive && (
                        ...
                        <div className="w-32 shrink-0 flex flex-col relative z-10">
                          <span className={`text-[22px] font-semibold tracking-tight ${isLive ? "text-emerald-600 dark:text-emerald-400" : "text-slate-800 dark:text-slate-100"}`}>
                            {apt.displayTime}
                          </span>
                        ...
                        <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-xs font-semibold text-slate-500 mt-1 uppercase tracking-wider truncate bg-slate-100 dark:bg-slate-800 w-max px-2 py-0.5 rounded-md">
                                {apt.type}
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="w-32 shrink-0 relative z-10 flex lg:justify-center">
                          <span className={`inline-flex items-center justify-center w-full max-w-[120px] px-3 py-1.5 rounded-lg text-[10px] font-bold tracking-widest uppercase shadow-sm border transition-colors ${apt.status?.toLowerCase() === "scheduled" || apt.status?.toLowerCase() === "upcoming"
                              ? "bg-teal-50 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400 border-teal-200 dark:border-teal-800/50"
                              : apt.status?.toLowerCase() === "completed"
                                ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800/50"
                                : apt.status?.toLowerCase() === "active" || apt.status?.toLowerCase() === "pending"
                                  ? "bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-300 border-amber-300 dark:border-amber-700/60"
